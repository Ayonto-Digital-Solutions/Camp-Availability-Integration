...
class AS_CAI_Markdown_Parser {
	...
	public function parse( $markdown ) {
		...
		$html = preg_replace_callback( '/```([a-z]*)\n(.*?)\n```/s', function ( $matches ) use ( &$code_blocks ) {
			...
			$code_blocks[ $placeholder ] = '<pre style="background:#1f2937;color:#e5e7eb;padding:16px;border-radius:8px;overflow-x:auto;margin:16px 0;"><code class="language-' . esc_attr( $language ) . '">' . $code . '</code></pre>';
			return "\n" . $placeholder . "\n";
		}, $html );
